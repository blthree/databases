...
def test_aioodbc_pool_size():
    backend = MSSQLBackend("postgresql+aioodbc://localhost/test_database1?min_size=1&max_size=20")
    kwargs = backend._get_connection_kwargs()
...
def test_aioodbc_ssl():
    backend = MSSQLBackend("postgresql+aioodbc://localhost/test_database1?ssl=true")
    kwargs = backend._get_connection_kwargs()
    assert kwargs["ssl"] == True
